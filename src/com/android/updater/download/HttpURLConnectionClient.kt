...
class HttpURLConnectionClient internal constructor(url: String?, destination: File,
                                                   progressListener: DownloadClient.ProgressListener?,
                                                   callback: DownloadCallback,
                                                   useDuplicateLinks: Boolean) : DownloadClient {
    ...
    private fun downloadFileInternalCommon(resume: Boolean) {
        ...
        if (resume) mDownloadThread!!.interrupt()
        ...
        private fun calculateSpeed(justResumed: Boolean) {
            ...
            if (delta > 500) {
                ...
                if (mSpeed == -1L) {
                    ...
                    val mLastSpeed = mSpeed
                    mSpeed = (mSpeed * 3 + curSpeed) / 4
                    val mSmoothing = 0.005
                    mSpeed = (mSmoothing * mLastSpeed + (1 - mSmoothing) * mSpeed).toLong()
                }
                mLastMillis = millis
                mCurSampleBytes = mTotalBytesRead
            }
            ...
            while (true) {
                try {
                    ...
                    if (duplicates != null && !duplicates.isEmpty()) {
                        ...
                        if (mDownloadThread == null || mDownloadThread!!.mCancelled) return
                        throw e
                    }
                }
            }
