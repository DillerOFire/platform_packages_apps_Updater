...
class UpdaterReceiver : BroadcastReceiver() {
    ...
    companion object {
        const val ACTION_INSTALL_REBOOT = "com.android.updater.action.INSTALL_REBOOT"
        ...
        private fun showUpdateFailedNotification(context: Context) {
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val buildDate = StringGenerator.getDateLocalizedUTC(context,
                    DateFormat.MEDIUM, preferences.getLong(Constants.PREF_INSTALL_NEW_TIMESTAMP, 0))
            val buildInfo = context.getString(R.string.list_build_version_date,
                    BuildInfoUtils.buildVersion, buildDate)
