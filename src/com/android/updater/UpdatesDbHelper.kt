...
class UpdatesDbHelper(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    private fun changeUpdateStatus(selection: String, selectionArgs: Array<String>,
                                   status: Int) {
        val db = writableDatabase
        val values = ContentValues()
    ...
    fun getUpdates(selection: String?, selectionArgs: Array<String?>?): List<Update> {
        ...
        val cursor = db.query(UpdateEntry.TABLE_NAME, projection, selection, selectionArgs,
                null, null, sort)
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                update.type = cursor.getInt(index)
                index = cursor.getColumnIndex(UpdateEntry.COLUMN_NAME_VERSION)
                update.version = cursor.getString(index).orEmpty()
